...
class ControllerPaymentGlobalPayments extends Controller {
	...
	public function index() {
		...
		if ($data['checkout'] == 'hpp') {
			...
			$servicesConfig->hostedPaymentConfig->responseUrl = $this->url->link('payment/globalpayments/apmRedirect', '', true);
			$servicesConfig->hostedPaymentConfig->statusUpdateUrl = $this->url->link('payment/globalpayments/apmUpdateOrderStatus', '', true);
			...
			try {
				...
				$this->model_payment_globalpayments->addOrder($this->session->data['order_id'], $response['ORDER_ID']);
				...
				$this->error['warning'] = $exception->responseCode . ' ' . $exception->responseMessage . ' ' . sprintf($this->language->get('error_payment'), $this->url->link('information/contact', '', true));
			}	
		}
	...
	public function api() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {							
			...
			try {
				...
				$this->model_payment_globalpayments->addOrder($this->session->data['order_id'], $orderId);
	...
	public function apmRedirect() {		
		...
		if (isset($this->request->request['ORDER_ID']) && isset($this->request->request['RESULT']) && isset($this->request->request['MESSAGE'])) {		
			...
			$order = $this->model_payment_globalpayments->getOrderByCode($orderId);
			...
			if ($order_info['order_status_id']) {
				if ((($settlement_method == 'auto') && ($order_info['order_status_id'] == $setting['order_status']['success_settled']['id'])) || (($settlement_method != 'auto') && ($order_info['order_status_id'] == $setting['order_status']['success_unsettled']['id']))) {
					...
					$data['redirect_url'] = $this->url->link('payment/globalpayments/apmFailurePage', '', true);
				}
			} else {				
				$data['redirect_url'] = $this->url->link('payment/globalpayments/apmInterimPage', '', true);
			}
			
			$this->response->setOutput($this->load->view('payment/globalpayments/apm_redirect', $data));
		}
	}
	...
	public function apmUpdateOrderStatus() {
		...
		if (isset($this->request->request['orderid']) && isset($this->request->request['result']) && isset($this->request->request['message'])) {		
			...
			if ($responseCode == '00') {
				...
				$this->model_payment_globalpayments->log($this->request->request, $responseCode . ' ' . $responseMessage);
			}
		}
	}
	...
	public function apmInterimPage() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/globalpayments/apmInterimPage', '', true)
		);
		...
		$data['apm_check_order_status_url'] = $this->url->link('payment/globalpayments/apmCheckOrderStatus', '', true);
		...
		$this->response->setOutput($this->load->view('payment/globalpayments/apm_interim_page', $data));
	}
		...
		if (isset($this->session->data['order_id'])) {			
			...
			if ($order_info['order_status_id']) {
				if ((($settlement_method == 'auto') && ($order_info['order_status_id'] == $setting['order_status']['success_settled']['id'])) || (($settlement_method != 'auto') && ($order_info['order_status_id'] == $setting['order_status']['success_unsettled']['id']))) {
					...
					$this->response->redirect($this->url->link('payment/globalpayments/apmFailurePage', '', true));
				}
			} else {				
				$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('config_order_status_id'));
				
				$this->response->redirect($this->url->link('payment/globalpayments/apmFinalPage', '', true));
			}
		}
	}
	...
	public function apmFailurePage() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_title'),
			'href' => $this->url->link('payment/globalpayments/apmFailurePage', '', true)
		);
		...
		$this->response->setOutput($this->load->view('payment/globalpayments/apm_failure_page', $data));
	}
	...
	public function apmFinalPage() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_title'),
			'href' => $this->url->link('payment/globalpayments/apmFinalPage', '', true)
		);
		...
		$this->response->setOutput($this->load->view('payment/globalpayments/apm_final_page', $data));
	}
